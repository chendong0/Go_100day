...
func main() {
	/*
		OOP中的继承性:
				如果两个类(class)存在继承关系,其中一个是子类,另一个做为父类,那么:
	...
	*/
	//1.创建Person类型
	p1 := Person{name: "陈自强", age: 31}
	fmt.Println(p1.name, p1.age) //父类对象, 访问父类的字段属性
	p1.eat()                     //父类对象,访问父类的方法


	//2.创建Student类型
	s1 := Student{Person{"carfield",2},"千锋教育"}
...
//3.方法
func (p Person) eat() {
	fmt.Println("父类的方法,吃窝窝头,指点江山...")

}
func (s Student) study() {
	fmt.Println("子类新增的方法, 学生要学习知识啦")

}
func (s Student) eat() {
	fmt.Println("子类重写的方法:学习学习再学习")
}
