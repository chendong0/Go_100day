...
func main() {
	/*
			...
			2.package声明的包和对应目录名可以不一致.但习惯上还是写成一致的
		package声明的包对应目录下的名要全部一直,这个卡了10分钟.
			3.包可以嵌套(定义之后还可以定义子包)
	...
	*/
	...
	p1 := person.Person{Name: "王富贵", Sex: "男"}
	//person.Person{},可以简化为导入包p "l_package/person",就可以使用p.Person{ }
	//Age,因为导入包时,age首字母小写不能被调用.
	...
	/*
	   ...
	   4.执行顺序
	   		...
	   		7._操作,其实是引入该包,而不直接使用包力铭的函数,仅仅是调用了该包里的init()
